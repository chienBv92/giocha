...
$(function () {
    ...
    $(document).on('focusout', 'input.hour', function () {
        var data = WebDuhoc.utility.validPositiveNumeric($(this).val()) ? $(this).val() : '00';
        $(this).val(data);
    });
    ...
    $(document).on('focusout', '.tell-no', function () {
        var data = WebDuhoc.utility.validPositiveNumeric($(this).val()) ? $(this).val() : '';
    ...
    $(document).on('focusout', '.negative-money', function () {
        if (this.value.length > 0) {
            var content;
            content = WebDuhoc.utility.Convert_JPCharacters(this.value, 'an');

            var data = WebDuhoc.utility.validNegativeNumeric(content.replace(/,/g, '')) ? content.replace(/,/g, '') : '0';
            ...
            data = WebDuhoc.utility.convertMoneyToInt(data, true).toString();
    ...
    $(document).on('focusout', '.money', function () {
        var content;
        content = WebDuhoc.utility.Convert_JPCharacters($(this).val(), 'n');

        var value = WebDuhoc.utility.convertMoneyToInt(content.split('.')[0].replace('-', ''));
        var data = WebDuhoc.utility.validPositiveNumeric(value.toString()) ? parseInt(value) : 0;
    ...
    $(document).on('focusout', '.numeric', function () {
        ...
        var data = WebDuhoc.utility.validPositiveNumeric(content) ? content : '0';
    ...
    $(document).on('focusout', '.negative-money-max', function () {
        ...
        var data = WebDuhoc.utility.validNegativeNumeric(this.value) ? this.value : '0';
